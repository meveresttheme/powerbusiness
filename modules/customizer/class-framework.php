...
if( !class_exists( 'Power_Business_Customizer' ) ){
	class Power_Business_Customizer extends Power_Business_Customizer_Sanitizer{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'power-business' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'power-business' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'power-business' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'power-business' );
				self::add_error( $err );
				$flag = false;
			}
		...
		public static function set( $args ){
			...
			do_action( 'power_business_customizer_before_set', self::get_instance() );

			if( isset( $args[ 'panel' ] ) ){
				$panel_id = 'power-business-' . $args[ 'panel' ][ 'id' ];
			...
			$sections = apply_filters( 'power_business_customizer_sections', $sections, $panel_id );
			...
			foreach( $sections  as $section ){
				...
				if( $prefix_on_section ){
					$section_id = 'power-business-' . $section[ 'id' ];
				}else{
				...
				foreach( $section[ 'fields' ] as $field ){
					...
					if( isset( $field[ 'id' ] ) ){
						...
						if( array_key_exists( $field_id, self::$settings ) ){
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'power-business' ), $field[ 'id' ] );
							self::add_error( $msg );
						}
			...
			do_action( 'power_business_customizer_after_set', self::get_instance() );
		}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'power-business' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'power-business' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'power-business' ),	
					esc_html__( 'section', 'power-business' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'power-business' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'power-business' );
				self::add_error( $msg, 2 );
			}

			if( !isset( $args[ 'fields' ] ) ){

				$msg = esc_html__( 'Fields are missing.', 'power-business' );
				self::add_error( $msg, 2 );
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'power-business' ),	
					esc_html__( 'fields', 'power-business' ),
					esc_html__( 'Must be a an array', 'power-business' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Power Business Customizer:', 'power-business' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		*/
	    public static function get_defaults(){ 
	    	$def = apply_filters(  'power_business_customizer_get_defaults', self::$defaults, self::get_instance() );
	    	return $def;
	    }
		...
		*/
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    'title' => empty( $panel[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'power-business' ) : $panel[ 'title' ],
			);
	        ...
	        return apply_filters( 'power_business_customizer_get_panel_arg', $args, $panel );
		}
		...
		*/
		public static function get_setting_arg( $field ){
			...
			return apply_filters( 'power_business_customizer_get_setting_arg', $args, $field );
		}
		...
		*/
		private static function get_control_arg( $section, $field, $prefix_on_section = true ){
			...
			if( is_array( $section ) && isset( $section[ 'id' ] ) ){
				$args[ 'section' ] = $prefix_on_section ? 'power-business-' . $section[ 'id' ] : $section[ 'id' ];
			}
			...
			if( isset( $field[ 'active_callback' ] ) ) {
	            $args[ 'active_callback' ] = 'power_business_' . $field[ 'active_callback' ];
	        }
			...
			return apply_filters( 'power_business_customizer_get_control_arg', $args, $section, $field );
		}
		...
		*/
		private static function get_partial_arg( $partial ){
			...
			return apply_filters( 'power_business_customizer_get_partial_arg', $args, $partial );
		}
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'power-business' ) : $section[ 'title' ],
			);
			...
			return apply_filters( 'power_business_customizer_get_section_arg', $args, $panel_id, $section );
		}
		...
		*/
		public static function register( $wp_customize ){
 			...
 			do_action(  'power_business_customize_register_start', self::get_instance(), $wp_customize );
			...
			do_action( 'power_business_customize_register_end', self::get_instance(), $wp_customize );	
...
add_action( 'customize_register', array( Power_Business_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'power_business_get' ) ):
	/**
	 ...
	 * @package Power Business WordPress Theme
	 */
	function power_business_get( $id ){
		$id = 'power-business-' . $id;
		return Power_Business_Customizer::get( $id );
	}
