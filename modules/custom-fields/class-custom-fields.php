...
if( !class_exists( 'Power_Business_Custom_Fields' ) ){
	class Power_Business_Custom_Fields extends Power_Business_Helper{
		...
		*/
		...
		public static function render_field( $field ){
			...
			switch( $field[ 'type' ] ){
				...
				case 'radio':
					?>
					<div class="components-base-control__field power-business-meta-radio">
						<?php  
							self::label( $field );
				...
				case 'select':
					...
					?>
					<div class="components-base-control__field power-business-meta-select">
						<?php  
							self::label( $field );
							self::description( $field );
						?>
				...
				case 'checkbox':
					?>
					<div class="components-base-control__field power-business-meta-checkbox">
						<input 
							type="checkbox" 
				...
				default:
					?>
					<div class="components-base-control__field power-business-meta-text">
						<?php  
							self::label( $field );
							self::description( $field );
						?>
		...
		public function render_meta_box( $post, $box ){
			...
			if( !is_array( $box[ 'args' ] ) )
				return;
			wp_nonce_field( 'power_business_meta_nonce', 'power_business_name_meta_nonce' );
			?>
			<div class="power-business-meta-box-wrapper">
				<?php 
					$count = 0; 
					foreach( $box[ 'args' ] as $id => $field ): 
						$id = 'power-business-' . self::uglify( $id );
				?>
					<div id="<?php echo esc_attr( $id ); ?>-wrapper" class="components-base-control__field power-business-meta-box-item">
						<?php 
							$field[ 'id' ]    = $id;
							$field[ 'post' ]  = $post;
		...
		public function save( $post_id ){
			...
			if ( $is_autosave || $is_revision || empty( $p ) || ! isset(  $p[ 'power_business_name_meta_nonce' ] ) || ! wp_verify_nonce( $p[ 'power_business_name_meta_nonce' ], 'power_business_meta_nonce' )) {
				return;
			}
